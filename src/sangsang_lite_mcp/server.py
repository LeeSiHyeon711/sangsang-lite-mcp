...
★ 최종 결과카드에는 design_first_experiment의 **readiness_summary 문자열을 맨 위에 그대로 출력**한다
(검증 준비도 점수 + 세부 4항목 + 한 줄 진단). 이건 내부 데이터가 아니라 사용자에게 보여줄 결과의 일부다 — 절대 생략하지 말 것.
그 아래에 균열점·첫 검증 미션·성공 기준·실패 신호·아직 만들지 말아야 할 것을 이어서 보여준다.
점수는 '아이디어 성공 가능성/사업성'이 아니라 '검증 가능한 형태로 얼마나 정리됐는가'다. 점수가 낮아도 '나쁜 아이디어'가 아니라
'아직 검증 가능한 형태로 덜 정리됨'으로 표현한다(단정 표현 금지).
"""
